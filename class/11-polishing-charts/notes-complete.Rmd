...
Use the `wildlife_costs` data frame to create the chart on the slides.
...
myplot <- ggplot(wildlife_impacts_2016) +
  geom_point(
...
Here's the chart with the `"RdYlBu"` ColorBrewer palette:
...
Here's the chart with the `"inferno"` palette from the **viridis** library:
...
Finally, here's the chart with a custom color palette: 

```{r, fig.width=7, fig.height=5, fig.align='center'}
colorPalette <- c('#a0522d', '#522da0', '#2da052')
...
Hint: Use `geom_text()` or `annotate()` to insert the year labels:
