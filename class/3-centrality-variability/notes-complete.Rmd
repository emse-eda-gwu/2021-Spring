...
marathon %>%
    ...
    filter(Overall < 500) %>%
    ggplot() +
    geom_boxplot(aes(x = `Official Time`, y = reorder(Country, `Official Time`)))

wildlife_impacts %>%
    filter(state != "N/A") %>%
    count(operator, state) %>%
    ggplot() +
    geom_col(aes(x = n, y = reorder(state, n), fill = operator))
```
...
candy_rankings %>%
  ggplot() +
  geom_col(aes(x = winpercent, y = reorder(competitorname, winpercent)))
