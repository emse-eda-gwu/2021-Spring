...
wildlife_impacts %>%
  ...
  mutate(
    ...
    phase_of_flt = fct_recode(phase_of_flt,
                              "All Other" = "Other")) %>%
  ggplot() +
...
```{r wildlife_bars_stacked, fig.width=4, fig.height=5}
ggplot(wildlife_summary) +
  geom_col(aes(x = "", y = n, fill = time_of_day),
           width = 0.7, alpha = 0.8) +
  scale_y_continuous(
    expand = expansion(mult = c(0, 0.05))) +
  theme_minimal_hgrid() +
...
wildlife_summary %>%
  ggplot() +
  geom_col(aes(x = n, y = reorder(time_of_day, n)),
           width = 0.7, alpha = 0.8) +
  scale_x_continuous(
    expand = expansion(mult = c(0, 0.05))) +
  theme_minimal_vgrid() +
...
wildlife_summary %>%
  ...
  geom_col(aes(x = "", y = n, fill = time_of_day),
           width = 0.7, alpha = 0.8) +
  coord_polar(theta = "y") +
  geom_text(aes(x = 1.2, y = n, label = label),
            color = "black", size = 4,
...
```{r wildlife_waffle, fig.height=5, fig.width=3}
# Divide the number of incidents by 100
wildlife_summary %>%
  mutate(n = n / 100) %>%
  ggplot() +
  geom_waffle(aes(fill = time_of_day, values = n),
  ...
  labs(fill = 'Time of day',
       x = NULL,
       y = NULL,
       title = str_c('Wildlife impact incidents by time of day\n',
                     '(1 square = 100 incidents)'))
```
